...
/**
 * logging.mod.ts: @amethyst/standard Logging API
 */
export class Logging {
  private static pretty = pretty.default({
    colorize: true,
  });
  public static cli = pino({
    name: 'cli',
  }, this.pretty);
  public static application = pino({
    name: 'application',
    transport: {
      target: './lib/rotating.ts',
      options: {},
    },
  }, this.pretty);
  public static backend = pino({
    name: 'backend',
  });
  public static network = pino({
    name: 'network',
  });
}

Logging.application.warn('x');
